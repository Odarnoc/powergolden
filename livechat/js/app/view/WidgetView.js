...
(function(app, $, config)
{
    var WidgetView = app.WidgetView = Backbone.View.extend({
        ...
        postMessage : function(data, callback)
        {
            ...
            window.parent.postMessage(data, document.location.origin.replace(/\/\/(?!www\.)/, '//www.'));
            window.parent.postMessage(data, document.location.origin.replace('//www.', '//'));
        ...
        initFramesCommunication : function()
        {
            ...
            {
                if(!evt.originalEvent.data || evt.originalEvent.origin !== document.location.origin) return;
